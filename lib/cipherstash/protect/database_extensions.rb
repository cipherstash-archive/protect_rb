module CipherStash
  module Protect
    module DatabaseExtensions
      def self.install
        CipherStash::Protect::Logger.info("Installing database extension.....")

        ActiveRecord::Base.connection.execute(install_script)

        CipherStash::Protect::Logger.info("Database extension installed.")
      end

      def self.uninstall
        CipherStash::Protect::Logger.info("Uninstalling database extension.....")

        ActiveRecord::Base.connection.execute(uninstall_script)

        CipherStash::Protect::Logger.info("Database extension uninstalled.")
      end
